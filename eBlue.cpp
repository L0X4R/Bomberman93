...
eBlue::eBlue()
{
	...
	if (graphicID != -1 || rm != nullptr || vm != nullptr)
	{
		...
		GOOD("ENEMIGO INSTANCIADO! [BLUE ENEMY]");
	}
	else
	{
		ERROR("NO SE HA PODIDO INSTANCIAR AL ENEMIGO. [BLUE ENEMY]");
		exit(1);
	}
